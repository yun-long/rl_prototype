...
class GaussianPolicy_MLP(object):
    ...
    def __init__(self, sess, sizes, activations=None, init_sigma=1.):
        ...
        self.mlp = MLP(sizes=sizes, activations= activations)
    ...
    def predict(self, state):
        return np.squeeze(self.sess.run(self.pred_action, {self.mlp.x : np.asmatrix(state)}), axis=0)

    def get_log_proba(self, state, action):
        return self.sess.run(self.log_prob, {self.mlp.x: state, self.actions: action})

    def get_mean(self, state, action):
        return self.sess.run(self.norm_dist, {self.mlp.x: state, self.actions: action})
