...
class Pend2dBallThrowDMP():
    ...
    def getReward (self, theta):
        ...
        # policy = u = data_traj[1] = f(q_des) = f(f(theta))
    ...
    def animate_fig(self, theta, lambda_coeff):
        q_des = self.getDesiredTrajDMP(np.reshape(theta,(-1,self.numBasis,)).T)
        data_traj = self.simulateSystem(q_des)
        q = data_traj[0]
        b = data_traj[2]
        ...
        for i in range(1,100,1):
            ...
            thisx = [0, mp1[0], mp2[0]]
            thisy = [0, mp1[1], mp2[1]]
            ax.plot(thisx, thisy, marker='*', markersize=10)
            ax.plot(b[t[i] - 1, 0], b[t[i] - 1, 1], marker='o', markersize=15)
            ax.set_yticks([])
            ax.set_xticks([])
            ax.axis((-dim, dim, -dim, dim))
            ax.add_artist(goalCircle)
            frame = self.fig2data(figure)
            frames.append(self.fig2data(figure))
            plt.close('all')
        imageio.mimsave('example_{0}.gif'.format(lambda_coeff), frames, fps=10)
